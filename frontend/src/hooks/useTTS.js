...
export function useTTS() {
  const [isPlaying, setIsPlaying] = useState(tts.isPlaying)
  const [isLoading, setIsLoading] = useState(tts.isFetching)
  ...
  useEffect(() => {
    ...
    if (savedProvider) tts.setProvider(savedProvider)
    if (savedVoice) tts.setVoice(savedVoice)
    if (savedSpeed) tts.setSpeed(parseFloat(savedSpeed))
    if (savedPitch) tts.setPitch(parseFloat(savedPitch))
