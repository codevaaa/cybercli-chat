...
import { Star, MessageSquare, Send, Check, Loader2, Lock } from 'lucide-react'
...
// Hardcoded seed testimonials shown only when the DB returns nothing
const SEED_TESTIMONIALS = [
  {
    _id: 'seed-1',
    name: 'Rishabh Thakur',
    initials: 'RT',
    quote: 'Codeva has completely changed how I approach security research. The multi-model council mode is unlike anything else — I get five different AI perspectives on a single vulnerability in seconds.',
    role: 'Cybersecurity',
    company: 'Codeva',
    stars: 5,
    accentColor: '#D97757',
  },
  {
    ...
    name: 'Abhay Pandey',
    initials: 'AP',
    quote: 'The developer experience is incredible. Switching between models without leaving the interface, voice chat, and the Kali_Kal engine for uncensored analysis — Codeva is the only AI platform I need.',
    role: 'Developer',
    company: '',
    stars: 5,
    accentColor: '#7C3AED',
...
export default function TestimonialsSection() {
  ...
  useEffect(() => {
    async function fetchTestimonials() {
      try {
        ...
        // Filter out old placeholder/seed entries by name
        const filtered = (data || []).filter(t =>
          !['Rishabh Cyber', 'abhay pandey'].includes((t.name || '').toLowerCase().trim())
        )
  ...
  // Which testimonials to display — live DB data, or seed if empty
  const displayedTestimonials = testimonials.length > 0 ? testimonials : SEED_TESTIMONIALS

  return (
    <section className="section-padding py-24 lg:py-32 border-y border-border-subtle relative overflow-hidden bg-background-primary">
        ...
        <AnimatePresence>
          {showForm && (
            ...
            >
              ...
              {!user ? (
                <div className="card-glass p-8 border border-white/[0.08] text-center">
                  ...
                  <h3 className="text-base font-bold text-white mb-2">Sign in to leave a review</h3>
                  <p className="text-sm text-foreground-muted mb-5">
                    Create a free account to share your Codeva experience with the community.
                  </p>
                  <a
                    href="/auth/signup"
                  ...
                  {submitSuccess ? (
                    ...
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        />
                        <p className="text-[11px] text-foreground-muted mt-1">{quote.trim().length}/500 characters</p>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                          <label className="block text-xs font-bold text-foreground-muted uppercase tracking-wider mb-2">Company <span className="font-normal text-foreground-muted/60">(optional)</span></label>
                          <input
                            type="text"
                            value={company}
                      ...
                      <div className="pt-2 flex justify-end">
                        <motion.button
                          ...
                          className="px-6 py-2.5 rounded-xl bg-accent text-white font-bold text-sm flex items-center gap-2 hover:bg-accent/90 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
                        >
                          {submitting ? (
                            <>
